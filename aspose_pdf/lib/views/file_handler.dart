...
class _FileHandlerState extends State<FileHandler> {
  final _endpoint = 'https://api.aspose.cloud';
  String _fileName;
  String _path;
  ...
  Future<void> _authorize() async {
    final http.Response response = await http.post(
      '$_endpoint/connect/token',
      body: {
        'client_id': '220de38b-0189-4286-b8ec-bcf00515502c',
        'client_secret': '11a215a4bdfe6d6734cc2cd39083e144',
        'grant_type': 'client_credentials'
      },
    );
    if (response.statusCode == 200 || response.statusCode == 201) {
      final String data = json.decode(response.body)['access_token'];
  ...
  Future<void> _uploadFile(file, filename, extension) async {
    ...
    var uri =
        Uri.parse("$_endpoint/v3.0/pdf/storage/file/$path?storageName=test");
  ...
  Future<void> _downloadFile(context) async {
    ...
    var request = new http.Request('GET',
        Uri.parse('$_endpoint/v3.0/pdf/storage/file/$_path?storageName=test'));
    request.headers['Authorization'] = "Bearer $token";
